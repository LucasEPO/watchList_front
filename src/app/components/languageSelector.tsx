...
const LanguageSelector = () => {
  ...
  // Carregar o idioma do localStorage ao montar o componente
  useEffect(() => {
    ...
    if (savedLanguage && savedLanguage !== selectedLanguage) {
      ...
      // Definir inglês como padrão se nenhum idioma estiver salvo
  ...
  const handleChangeLanguage = (event: any, newValue: { id: string; name: string } | null) => {
    if (newValue && newValue.id !== selectedLanguage) {
      ...
      localStorage.setItem('language', newValue.id); // Salvar idioma escolhido no localStorage
      setSelectedLanguage(newValue.id); // Atualiza o estado com o novo idioma
      setLoading(false);
    }
  };
